...
public class DeleteCookie extends BlockingNativeCallableUnit {
    @Override
    public void execute(Context context) {
        BMap<String, BValue> cookieJar = (BMap<String, BValue>) context.getRefArgument(FIRST_PARAMETER_INDEX);
        BMap<String, BValue> cookieToBeDeleted = (BMap<String, BValue>) context.getRefArgument(SECOND_PARAMETER_INDEX);
        if (cookieJar.getNativeData(SERVER_COOKIES) != null) {
            ArrayList<BMap<String, BValue>> cookies = (ArrayList<BMap<String, BValue>>) cookieJar.getNativeData(
                    SERVER_COOKIES);
            cookies.removeIf(cookie -> cookie.equals(cookieToBeDeleted));
        }
    }
}
